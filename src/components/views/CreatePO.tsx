...
export default () => {
    ...
    const form = useForm<FormData>({
        ...
        defaultValues: {
            ...
            deliveryDays: undefined, // NEW FIELD
            deliveryType: undefined, // NEW FIELD
    ...
    useEffect(() => {
        if (mode === 'revise') {
            form.reset({
                ...
                indents: [],
                terms: [],
                deliveryDays: undefined, // ADD THIS
                deliveryType: undefined, // ADD THIS
            });
        } else {
            form.reset({
                poNumber: generatePoNumber(poMasterSheet.map((p) => p.poNumber)),
    ...
    useEffect(() => {
        ...
        if (mode === 'revise' && po) {
            ...
            form.setValue('deliveryDays', po.deliveryDays || 0); // ADD THIS
            form.setValue(
                'deliveryType',
                po.deliveryType === 'for' || po.deliveryType === 'exfactory'
                    ? po.deliveryType
                    : undefined
            ); // ADD THIS
            form.setValue(
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <select
                                                        className="h-9 w-full rounded-md border border-gray-300 px-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                                                        value={field.value || ""}
                                                        onChange={(e) => field.onChange(e.target.value)}
                                                    >
                                                        <option value="">-- Select Type --</option>
                                                        <option value="for">FOR</option>
                                                        <option value="exfactory">Ex-Factory</option>
                                                    </select>
                                                </FormControl>
                                            </FormItem>
                                        )}
