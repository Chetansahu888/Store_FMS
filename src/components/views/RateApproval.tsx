...
async function onSubmit(values: z.infer<typeof schema>) {
    try {
        ...
        const updatedRows = filtered.map((prev) => ({
    ...
    async function onSubmitHistoryUpdate(values: z.infer<typeof historyUpdateSchema>) {
        try {
            ...
            const updatedRows = filtered.map((prev) => ({
                rowIndex: prev.rowIndex,  // ✅ Required for backend to know which row to update
                approvedRate: values.rate,  // ✅ Only send the field you want to update
            }));
